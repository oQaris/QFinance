...
class PolicyGradient:
    ...
    def _setup_train(self, env: PortfolioOptimizationEnv, policy, batch_size, lr, optimizer):
    ...
    def train(self, episodes=100):
        ...
        for i in tqdm(range(1, episodes + 1)):
            obs = self.train_env.reset()[0]['price_data']
            ...
            while not done:
                ...
                # run simulation step
                next_obs, revard, done, _, info = self.train_env.step(action)
                next_obs = next_obs['price_data']
                self.train_pvm.add(info["real_weights"])
    ...
    def test(
        ...
        obs = self.test_env.reset()[0]['price_data']
        ...
        while not done:
            ...
            # run simulation step
            next_obs, _, done, _, info = self.test_env.step(action)
            next_obs = next_obs['price_data']
            self.test_pvm.add(info["real_weights"])
